...
def main() -> None:
    ...
    mlflow.sklearn.autolog(log_datasets=False)  # Disable dataset logging to avoid Series warning

    sst5 = fetch_openml("sst5")
    X = sst5.data["text"] 
    y = sst5.target.map({"very negative": 0, "negative": 1, "neutral": 2, "positive": 3, "very positive": 4})
